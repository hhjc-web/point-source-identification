...
def trainingdata(N_f, N_v):
    ...
    grad_r = autograd.grad(solution(g).real, g, torch.ones([g.shape[0], 1]).to(device), retain_graph=True, create_graph=True)[0]
    grad_i = autograd.grad(solution(g).imag, g, torch.ones([g.shape[0], 1]).to(device), retain_graph=True, create_graph=True)[0]
...
class Sequentialmodel(nn.Module):
    ...
    def loss_BC(self, x, y):
        for i in range(self.n_pred):
            ...
            y = y - PHI
        loss_v = nsquare * torch.norm(self.forward_r(x) + self.forward_i(x) * 1j - y)**2 / N_v
        return loss_v
    ...
    def loss_N(self, x, y):
        ...
        for i in range(self.n_pred):
            ...
            PHI = self.c[i] * math.e**(k*r*1j)/(4 * math.pi * r)
            grad_r = autograd.grad(PHI.real, g, torch.ones([N_v, 1]).to(device), retain_graph=True,
                                   create_graph=True)[0]
            grad_i = autograd.grad(PHI.imag, g, torch.ones([N_v, 1]).to(device), retain_graph=True,
                                   create_graph=True)[0]
        ...
        v_x_r = autograd.grad(v_r, g, torch.ones([N_v, 1]).to(device), retain_graph=True, create_graph=True)[0]
        yn_r = torch.sum(v_x_r * outernormal, dim = 1, keepdim=True)
        v_i = self.forward_i(g)
        v_x_i = autograd.grad(v_i, g, torch.ones([N_v, 1]).to(device), retain_graph=True, create_graph=True)[0]
        yn_i = torch.sum(v_x_i * outernormal, dim=1, keepdim=True)
        loss_v = nsquare * torch.norm(yn_r + yn_i * 1j - y)**2 / N_v
        return loss_v
    ...
    def loss_PDE(self, x_to_train_f):
        ...
        loss_f = nvolume * torch.norm(f)**2 / N_f
        return loss_f
