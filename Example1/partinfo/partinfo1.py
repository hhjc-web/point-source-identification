...
def trainingdata(N_v, N_f):
    ...
    all_X_u_train = np.vstack([leftedge_x, rightedge_x])
    all_u_train = np.vstack([leftedge_u, rightedge_u])
    all_X_un_train = np.vstack([leftedge_x, rightedge_x, topedge_x, bottomedge_x])
    all_un_train = np.vstack([leftdiff_u, rightdiff_u, topdiff_u, bottomdiff_u])


    temp = np.sum(np.abs(all_X_un_train), axis=1)
    index = np.where(temp == 2)
    all_X_un_train = np.delete(all_X_un_train, index, 0)
    all_un_train = np.delete(all_un_train, index, 0)
    ...
    idx = np.random.choice(all_X_un_train.shape[0], N_v, replace=False)
    X_u_train = all_X_u_train
    u_train = all_u_train
    un_train = all_un_train
    X_un_train = all_X_un_train

    '''Collocation Points'''
    # N_f sets of tuples(x,t)
    X_f = lb + (ub - lb) * lhs(2, N_f)
    X_f_train = np.vstack((X_f, X_u_train))  # append training points to collocation points
    return X_f, X_u_train, u_train, X_un_train, un_train
...
N_v = 1000
N_f = 5000
X_f_train_np_array, X_u_train_np_array, u_train_np_array, X_un_train_np_array, un_train_np_array = trainingdata(N_v, N_f)
...
X_un_train = torch.from_numpy(X_un_train_np_array).float().to(device)
...
optimizer1 = optim.Adam([SSPINN.c,SSPINN.x], lr=3e-3,betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
max_iter = 10000
interval = 100
